...
func GoogleScrape(searchTerm, countryCode, languageCode string, proxyString interface{}, pages, count int, backoff int) ([]SearchResult, error) {
	results := []SearchResult{}
	resultCounter := 0
...
func main() {
	res, err := GoogleScrape("sunto ryota", "com", "en", nil, 1, 30, 10)
	if err == nil {
		for _, res := range res {
			fmt.Println(res)
		}
